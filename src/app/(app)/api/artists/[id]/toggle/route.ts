  ...
  try {
    ...
    if (!field || typeof value !== 'boolean') {
      return NextResponse.json(
        { error: 'Field and boolean value are required' },
        { status: 400 }
      )
    }

    // Only allow specific fields
    const allowedFields = ['public_profile_enabled', 'auto_publish', 'status']
    ...
    if (error) {
      console.error('[api/artists/toggle] Update error:', error)
      return NextResponse.json(
        { error: error.message },
        { status: 500 }
      )
    ...
    console.error('[api/artists/toggle] Error:', error)
    return NextResponse.json(
