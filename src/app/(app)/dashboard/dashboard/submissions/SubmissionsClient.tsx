...
export default function SubmissionsClient() {
  ...
  const handleApprove = async (submission: Submission) => {
    ...
    try {
      ...
      if (!res.ok) {
        setNotice({ type: 'error', message: data.error || 'Approve failed' })
        return
      }
  ...
  const handleReject = async (submission: Submission) => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        setNotice({ type: 'error', message: data.error || 'Decline failed' })
        return
      }
      await fetchSubmissions()
            ...
            {filteredSubmissions.map(sub => (
              ...
              >
                ...
                <div className="p-4 sm:p-6 space-y-4">
                  {sub.genre && <p className="text-sm"><span className="text-[var(--pf-text-muted)]">Genre:</span> {sub.genre}</p>}
                  {sub.city && <p className="text-sm"><span className="text-[var(--pf-text-muted)]">City:</span> {sub.city}</p>}
                  {sub.bio && <p className="text-sm text-[var(--pf-text-secondary)]">{sub.bio}</p>}
                    ...
                    <div className="space-y-2">
                      ...
                      {sub.tracks.map(track => (
                        <a
                          ...
                          className="flex items-center gap-2 text-sm text-[var(--pf-text-secondary)] hover:text-[var(--pf-orange)] transition"
                        >
                  ...
                  {sub.status === 'pending' && (
                    <div className="flex gap-2 pt-2">
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 rounded-lg bg-green-500/20 text-green-500 hover:bg-green-500/30 transition disabled:opacity-50"
                      >
                        <Check size={16} />
                      ...
                      <button
                        ...
                        disabled={processing === sub.id}
                        className="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500/20 text-red-500 hover:bg-red-500/30 transition disabled:opacity-50"
                      >
                        <X size={16} />
                        {processing === sub.id ? 'Processing...' : 'Decline'}
                      </button>
                    </div>
                  )}
                </div>
