            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
              {artistMerch.map((product) => (
                <Link 
                  ...
                  className="bg-[var(--pf-surface)] rounded-xl overflow-hidden border border-[var(--pf-border)] hover:border-[var(--pf-orange)] hover:shadow-lg hover:shadow-[var(--pf-orange)]/10 transition-all duration-200 group relative"
                  aria-label={`${product.name} by ${product.artist} - $${(product as any).price || 9.99}`}
                >
                  <div className="aspect-square relative overflow-hidden">
                    ...
                    {!product.inStock && (
                      <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                        <span className="bg-white text-black px-3 py-1 rounded-full text-sm font-bold">Sold Out</span>
                      </div>
                    )}
